...
public class PowerCalculator {
    ...
    public static void main(String[] args) {
        ...
        try{
            ...
            calculatePower(value, pow );
        }catch (InvalidInputException e){
            System.out.println("Ошибка: " + e.getMessage());
        }
    }

    public static int calculatePower(int value, int pow) throws InvalidInputException {
        if (pow < 0) throw new InvalidInputException("Число степени не может быть отрицательным");
        if (pow == 0) throw new InvalidInputException("Невозможно возвести в нулевую степень");
        if (value == 0) throw new InvalidInputException("Число возводимое в степень не может быть равно нулю");

        int result = (int) Math.pow(value, pow);
